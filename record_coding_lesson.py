...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record a shared coding lesson.")
    ...
    parser.add_argument("--negation-of", type=int,
                        help="id of the lesson this one negates/guards; binds the pair so both halves render together")
    parser.add_argument("--always-on", action="store_true",
                        help="ride every query as artifact taste, not only situational matches")
    ...
    try:
        with conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                ...
                RETURNING id, scope, project, title
                """,
                (
                    args.scope,
                    args.project,
            ...
            row = cur.fetchone()
            print(f"upserted coding_lesson id={row['id']} scope={row['scope']} project={row['project']} title={row['title']}")
    finally:
        conn.close()
