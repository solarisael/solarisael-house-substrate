...
def upsert(cur, a) -> dict:
    ...
    cur.execute(
        ...
        ON CONFLICT (title) WHERE lesson_key = 'audio' DO UPDATE SET
          ...
          negation_of = EXCLUDED.negation_of, tags = EXCLUDED.tags,
        ...
        (a.shape, a.stage or ["general"], a.title, a.lesson, a.trigger_context,
         a.example_cmd, a.tool, negation_of, a.tag, a.source_memory_path),
    )
    return cur.fetchone()
...
def main() -> int:
    ...
    p.add_argument("--env-file", default=str(Path(__file__).parent / ".env"))
