...
impl RememberRequest {
    pub fn validate(&self) -> Result<(), AppError> {
        ...
        if !room_re.is_match(&self.room) || self.room == "house" {
        ...
        if self.kind == "memory" {
            ...
            let unsupported = match self.kind.as_str() {
                "coding-lesson" => false,
                "project-lesson" => {
                    self.voice.is_some() || self.shape.is_some() || self.scope.is_some()
                }
                "writing-lesson" => {
...
pub async fn remember(
    ...
    tx.commit().await?;
    if req.backup {
        if let Err(e) = run_backup(pool, &cfg.database_url).await {
...
async fn run_backup(pool: &PgPool, database_url: &str) -> Result<(), String> {
    backup::run_post_write(pool, database_url)
        .await
        .map_err(|e| e.to_string())
}

fn normalize_threads(values: &[String]) -> Vec<String> {
    values
        .iter()
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
...
fn derive_dates(path: &str, primary_date: NaiveDate) -> Vec<NaiveDate> {
    ...
    for c in stitched.captures_iter(path) {
        if let (Ok(y), Ok(m), Ok(d), Ok(hour)) = (
            c[1].parse::<i32>(),
            c[2].parse::<u32>(),
            c[3].parse::<u32>(),
            c[4].parse::<u32>(),
            ...
            if hour < 24 {
                if let Some(x) = NaiveDate::from_ymd_opt(y, m, d) {
                    out.push(x + chrono::Duration::days(1));
                }
            }
        }
    }
...
pub fn cluster_is_stale(s: &ClusterStaleness, now: chrono::DateTime<chrono::Utc>) -> bool {
    ...
    let fraction =
        s.chunks_total > 0 && (s.chunks_since_build as f64 / s.chunks_total as f64) >= 0.05;
    let age =
        now.signed_duration_since(s.built_at.unwrap()).num_days() >= 7 && s.chunks_since_build > 0;
    count || fraction || age
}
